...
int main(int argc, char** argv)
{
  ...
  if (!filename_specified) trajFile = fopen("mRNA2.txt", "r");
  else
  {
  ...
  if (trajFile == NULL)
  {
    fprintf(stderr, "Error, file mRNA2.txt is missing.\n");
    exit(EXIT_FAILURE);
  }
  ...
  glutCreateWindow ( "mRNA 2 (t)- View cells" );
  init ();
...
void GenerateVideo(){
    std::cout << "Generate video" << std::endl;
  system("ffmpeg -framerate 10 -i SimuscaleFrame%d_mRNA2.png VideoOutput_mRNA2.mp4");
  system("rm -f *_mRNA2.png");
}
...
void paintGL()
{
    ...
    string thename = "SimuscaleFrame" + std::to_string(_frame) + "_mRNA2.png";
    stbi_flip_vertically_on_write(true);
    stbi_write_png(thename.c_str(), _width, _height, _nrChannels, _buffer.data(), stride);
}

void read_min_max_signals()
{
  FILE * minmaxfile = fopen("NormM2.txt", "r");
  if (minmaxfile == NULL)
  {
    printf("No file called NormM2.txt in current directory.\n");
  ...
  while (retval != 1)
  {
    if (retval == EOF) {printf("File NormM2.txt found but premature end of file.\n");return;}
...
void read_file_line()
{
  ...
  while (retval != 1)
  {
    ...
    if (fscanf(trajFile, "%c", &c) == 1)
    {
      ...
      else if (c=='!')
      {
        ...
        do
        {
          retval = fscanf(trajFile, "%c", &c); /* printf("%c", c); */
        } while ((c != '\n') && (retval != EOF));
        if (retval == EOF) {exit(0);}
        _line ++;
      }
  ...
  {
    fscanf(trajFile, "%d %f %f %f %f %f\n", &_mycells[i].id, &_mycells[i].position[0], &_mycells[i].position[1], &_mycells[i].position[2], &_mycells[i].radius, &signal);
  ...
  if (minclip) printf("Warning, at least one cell in pop1 had min color clipping at t=%f.\n", _time);
  if (maxclip) printf("Warning, at least one cell in pop1 had max color clipping at t=%f.\n", _time);
...
void init(void)
{
  ...
  _max_signal = _max_signal / _OnOffThreshold;  
...
void display(void)
{
  ...
  glColor3f (1.0, 1.0, 1.0);
  sprintf(mystring, "Signal");
  glWindowPos2i(330, 30); /* also sets current raster color to white */
  for (i = 0; i < strlen(mystring); i++)
    glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, mystring[i]);
...
{
  printf( "\n************* Chronos - Viewing program ************* \n\n" );
  printf( "\n\
Usage : %s -h\n\
   or : %s [options]\n\n\
\t-h or --help      : Display this screen\n\n\
Options (i : integer, d : double, s : string) :\n\n\
\t-f or --file  s   : View the simulation stored in file named s (default: mRNA2.txt)\n\
...
While the animation is playing, use the left button of the mouse to change the viewpoint,\n\
and the right button to zoom in or zoom out. Press R to reset the perspective. Press the \n\
space bar to pause the animation or to continue it. Press ESC to quit.\n\n",\
