...
int main(int argc, char** argv)
{
  ...
  if (!filename_specified) trajFile = fopen("mRNA1.txt", "r");
  else
  {
  ...
  if (trajFile == NULL)
  {
    fprintf(stderr, "Error, file mRNA1.txt is missing.\n");
    exit(EXIT_FAILURE);
  }
  ...
  glutCreateWindow ( "mRNA 1 (t)- View cells" );
  init ();
...
void GenerateVideo(){
  ...
  system("ffmpeg -framerate 10 -i SimuscaleFrame%d_mRNA1.png VideoOutput_mRNA1.mp4");
  system("rm -f *_mRNA1.png");
}
...
void paintGL()
{
    ...
    string thename = "SimuscaleFrame" + std::to_string(_frame) + "_mRNA1.png";
...
{
  FILE * minmaxfile = fopen("NormM1.txt", "r");
  if (minmaxfile == NULL)
  {
    printf("No file called NormM1.txt in current directory.\n");
  ...
  while (retval != 1)
  {
    if (retval == EOF) {printf("File NormM1.txt found but premature end of file.\n");return;}
...
\t-f or --file  s   : View the simulation stored in file named s (default: mRNA1.txt)\n\
