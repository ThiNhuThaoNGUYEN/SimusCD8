...
int main(int argc, char** argv)
{
  ...
  if (!filename_specified) trajFile = fopen("gene2.txt", "r");
  else
  {
  ...
  if (trajFile == NULL)
  {
    fprintf(stderr, "Error, file gene2.txt is missing.\n");
    exit(EXIT_FAILURE);
  }
  ...
  glutCreateWindow ( "Protein 2 (t)- View cells" );
  init ();
...
void GenerateVideo(){
  system("ffmpeg -framerate 10 -i SimuscaleFrame%d_Protein2.png VideoOutput_P2.mp4");
  system("rm -f *_Protein2.png");
}
...
void paintGL()
{
    ...
    string thename = "SimuscaleFrame" + std::to_string(_frame) + "_Protein2.png";
...
{
  FILE * minmaxfile = fopen("NormGene2.txt", "r");
  if (minmaxfile == NULL)
  {
    printf("No file called NormGene2.txt in current directory.\n");
  ...
  while (retval != 1)
  {
    if (retval == EOF) {printf("File NormGene2.txt found but premature end of file.\n");return;}
...
\t-f or --file  s   : View the simulation stored in file named s (default: gene2.txt)\n\
