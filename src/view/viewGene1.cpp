...
int main(int argc, char** argv)
{
  ...
  if (!filename_specified) trajFile = fopen("gene1.txt", "r");
  else
  {
  ...
  if (trajFile == NULL)
  {
    fprintf(stderr, "Error, file gene1.txt is missing.\n");
    exit(EXIT_FAILURE);
  }
  ...
  glutCreateWindow ( "Protein 1 (t)- View cells" );
  init ();
...
void GenerateVideo(){
  system("ffmpeg -framerate 10 -i SimuscaleFrame%d_Protein1.png VideoOutput_P1.mp4");
  system("rm -f *_Protein1.png");
}
...
void paintGL()
{
    ...
    string thename = "SimuscaleFrame" + std::to_string(_frame) + "_Protein1.png";
...
{
  FILE * minmaxfile = fopen("NormGene1.txt", "r");
  if (minmaxfile == NULL)
  {
    printf("No file called NormGene1.txt in current directory.\n");
  ...
  while (retval != 1)
  {
    if (retval == EOF) {printf("File NormGene1.txt found but premature end of file.\n");return;}
...
\t-f or --file  s   : View the simulation stored in file named s (default: gene1.txt)\n\
