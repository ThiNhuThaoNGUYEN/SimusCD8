...
void OutputManager::PrintStatsHeader() {
  ...
  const std::vector<std::string> names = Simulation::intercellular_signal_names(); 
  ...
  for ( auto signal : Simulation::using_signals() ) {
    fprintf(outputs_[STATS], "# %" PRId32 " : avg signal level %s\n", column_nbr++, names[static_cast<int16_t>(signal)].c_str());
  }
}
...
void OutputManager::PrintTrajectoryHeader(void) {
 ...
 fprintf(outputs_[TRAJECTORY], "# 1 : time\n");
  ...
  fprintf(outputs_[TRAJECTORY], "# 7: external radius \n");
  int16_t column_nbr = 8;

#ifdef PRINT_CELLTYPE
    fprintf(outputs_[TRAJECTORY], "# 8 : cell type\n");
    column_nbr++;
#endif
...
void OutputManager::PrintTrajectory(void) {
  ...
  for (auto cell : Simulation::pop().cell_list()) {
    ...
    fprintf(outputs_[TRAJECTORY],
            "%d " FL_FMT " " FL_FMT " " FL_FMT " " FL_FMT " " ,
            cell->id(),
            cell->pos_x(), cell->pos_y(), cell->pos_z(),
            cell->external_radius());

#ifdef PRINT_CELLTYPE
    fprintf(outputs_[TRAJECTORY],
            "%s ", Simulation::celltype_names().at(static_cast<int16_t>(cell->cell_type())).c_str());
#endif
...
void OutputManager::PrintNormalization(void) {
  ...
  for ( auto signal : Simulation::using_signals() ) {
    fprintf(normalization_file_,
            "%s " FL_FMT " " FL_FMT "\n",
            Simulation::intercellular_signal_names().at(static_cast<int16_t>(signal)).c_str(),
