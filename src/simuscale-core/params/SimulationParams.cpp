...
CellType SimulationParams::StrToCellType(const std::string cell_type_str) {
   size_t i = 0;
  while ( i < celltype_names_.size() ) {
    if ( cell_type_str == celltype_names_.at(i)) 
      return static_cast<CellType>(i);
      ++i;
...
InterCellSignal SimulationParams::StrToInterCellSignal(const std::string signal_str) {  
  size_t i = 0;
  while ( i < intercellular_signal_names_.size() ) {
    if ( signal_str == intercellular_signal_names_.at(i) ) 
      return static_cast<InterCellSignal>(i);
      ++i;
  }
