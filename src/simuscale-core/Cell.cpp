...
Coordinates<double> Cell::MotileDisplacement(const double& dt) {
...
void Cell::Load(gzFile backup_file) {
  ...
  move_behaviour_ =( (cell_type_ == NICHE)||(cell_type_ == APC)) ?
...
void Cell::AddMechForce(const Cell* other) {
  if ((cell_type() == NICHE)||(cell_type() == APC)) return; // don't move niche/APC cells
...
void Cell::AddChemCom(const Cell* other) {
  ...
  if (h > 0) { // if cells are touching one another
     ...
     double contact_area = M_PI * (mean_radius * h - h*h / 4);
    

     intrinsic_inputs_.Add(InterCellSignal::CYCLE_X,
                          contact_area * other->get_output(InterCellSignal::CYCLE_X));
    intrinsic_inputs_.Add(InterCellSignal::DEATH,
                          other->get_output(InterCellSignal::DEATH));

   if (other->cell_type() == LYMPHOCYTE) {
    intrinsic_inputs_.Add(InterCellSignal::DIFFERENTIATION_STATE, 0.);
}
   if (other->cell_type() == APC && cell_type()==LYMPHOCYTE) {
    intrinsic_inputs_.Add(InterCellSignal::APC_CONTACT, 1.);
    

//std::cout << "-----   APC cell is a neighbour! ------" << std::endl;
}

   if (other->cell_type() == LYMPHOCYTE && cell_type()==LYMPHOCYTE) {
    intrinsic_inputs_.Add(InterCellSignal::LYMPHOCYTE_CONTACT, 1.);

//std::cout << "-----   LYMPHOCYTES CONTACT! ------" << std::endl;
}
